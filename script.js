...
// Inicialização do Firebase (deve vir ANTES de qualquer uso do database)
...
// Tags registradas (incluindo professores) (carregadas ou inicializados)
let registeredTags = [];
...
function listenForDoorStatusChanges() {
  try {
    const statusRef = database.ref(
      `laboratorios/${currentLaboratory}/statusPorta`
    );
    statusRef.on(
      "value",
      (snapshot) => {
        try {
          ...
          if (data) {
            laboratories[currentLaboratory].isLocked = data.isLocked;
            updateLockStatus();
          }
        } catch (error) {
...
function renderActivityFeed() {
  ...
  feedContainer.innerHTML = activityData
    .map(
      (activity) => `
...
function changeLaboratory() {
  ...
  addActivity(
    "info",
    "Laboratório Alterado",
    `Laboratório ${currentLaboratory} selecionado`,
    "bi-building"
  );
  syncData();
}
...
function registerTeacherModal() {
  const modal = new bootstrap.Modal(
    document.getElementById("registerTeacherModal")
  );
  modal.show();
...
function confirmRegisterTeacher() {
  ...
  const coordinator = document
    .getElementById("registerTeacherCoordinator")
    .value.trim();
  const rfid = document
    .getElementById("registerTeacherRFID")
    .value.trim()
    .toUpperCase();
  ...
  if (teachers[rfid]) {
    alert(
      `O ID RFID "${rfid}" já está registrado para o professor ${teachers[rfid].name}.`
    );
    return;
  }
...
function confirmTeacherChange() {
  const teacherSelect = document.getElementById("teacherSelect");
  const rfid = teacherSelect.value;
  ...
  // Atualizar no banco
  const teacherRef = database.ref(`laboratorios/${currentLaboratory}/teacher`);
  teacherRef.set(teacher).then(() => {
    laboratories[currentLaboratory].teacher = teacher;
    updateTeacherInfo();
    addActivity(
      "info",
      "Professor Alterado",
      `Professor do laboratório ${currentLaboratory} alterado para ${teacher.name}`,
      "bi-person-fill"
    );
    syncData();
    const modal = bootstrap.Modal.getInstance(
...
function removeTeacher() {
  ...
  if (
    !confirm(
      `Tem certeza que deseja remover o professor ${teacherName} do Laboratório ${currentLaboratory}? Isso removerá apenas as aulas agendadas para este laboratório.`
    )
  ) {
    return;
  }
...
async function toggleLock() {
  try {
    ...
    const newLockStatus = !laboratories[currentLaboratory].isLocked;
    const updateData = {
      isLocked: newLockStatus,
      timestampUltimaAcao: Date.now(),
      ultimaAcaoMensagem: newLockStatus
        ? "Comando: Bloquear Porta"
        : "Comando: Desbloquear Porta",
    };
    await statusRef.set(updateData);
    laboratories[currentLaboratory].isLocked = newLockStatus;
    isLocked = newLockStatus;
    updateLockStatus();
    addActivity(
      "info",
      "Status do Laboratório Alterado",
      `Laboratório ${currentLaboratory} foi ${
        newLockStatus ? "trancado" : "destrancado"
      }`,
      newLockStatus ? "bi-lock-fill" : "bi-unlock-fill"
    );
    syncData();
  } catch (error) {
    console.error("Erro ao alterar status da fechadura:", error);
    alert("Erro ao alterar status da fechadura. Por favor, tente novamente.");
  }
}
...
function updateLockStatus() {
  ...
  if (!roomStatus || !lockBtn) return;

  const isLabLocked = laboratories[currentLaboratory].isLocked;

  // Atualizar texto e estilo do status
  roomStatus.textContent = isLabLocked ? "Trancado" : "Desbloqueado";
  roomStatus.className = isLabLocked ? "status-locked" : "status-unlocked";

  // Atualizar botão
  lockBtn.innerHTML = isLabLocked
    ? '<i class="bi bi-lock-fill me-2"></i>Destrancar Laboratório'
    : '<i class="bi bi-unlock-fill me-2"></i>Trancar Laboratório';
  lockBtn.className = isLabLocked ? "btn btn-danger" : "btn btn-success";
}
...
function handleRFIDEvent(data) {
  ...
  const detectedTag = rfid
    ? registeredTags.find((tag) => tag.id === rfid)
    : null;
  ...
  if (lab === currentLaboratory) {
    ...
    if (newIsLocked !== isLocked) {
      // Apenas atualiza se o status mudou
      ...
      if (isLocked) {
        ...
        addActivity(
          "danger",
          "Porta Trancada",
          `Porta do Laboratório ${lab} trancada${
            detectedTag ? ` por ${detectedTag.name} (${rfid})` : ""
          }`,
          "bi-lock-fill"
        );
      } else {
        ...
        addActivity(
          "success",
          "Porta Aberta",
          `Porta do Laboratório ${lab} aberta${
            detectedTag ? ` por ${detectedTag.name} (${rfid})` : ""
          }`,
          "bi-door-open-fill"
        );
    ...
    if (rfid) {
      addActivity(
        "warning",
        "RFID Detectado (Outro Lab)",
        `RFID ${rfid} detectado no Laboratório ${lab}, mas o laboratório atual é ${currentLaboratory}. Status: ${status}`,
...
function registerTag() {
  const tagId = prompt(
    "Por favor, insira o ID RFID da nova tag (ex: ABCD123):"
  );
  if (!tagId || tagId.trim() === "") {
  ...
  const userName = prompt(
    "Por favor, insira o nome do usuário para a nova tag RFID:"
  );
  ...
  if (userName && userName.trim()) {
    ...
    addActivity(
      "info",
      "Nova Tag Registrada",
      `Tag RFID ${newTag.id} registrada para ${userName}`,
      "bi-plus-circle-fill"
    );
              ...
              <tbody>
                ${activityData
                  ...
                  <tr>
                    ...
                    <td><span class="badge bg-${getActivityColor(
                      activity.type
...
function loadRegisteredTags() {
  ...
  tagsTable.innerHTML = registeredTags
    .map(
      (tag) => `
    <tr>
      ...
      <td>
        <span class="badge ${
          tag.status === "Active" ? "bg-success" : "bg-secondary"
        }">
          ${tag.status}
        </span>
      </td>
      <td>
        <span class="badge ${
          tag.type === "teacher" ? "bg-primary" : "bg-info"
        }">
...
function deleteTag(tagId) {
  if (confirm(`Are you sure you want to delete tag ${tagId}?`)) {
    ...
    addActivity(
      "warning",
      "Tag Deleted",
      `RFID tag ${tagId} has been removed`,
      "bi-trash-fill"
    );
  }
}
...
function updateActiveTags() {
  ...
  const activeCount = registeredTags.filter(
...
function updateTimestamps() {
  ...
  document.getElementById(
    "lastAccess"
  ).textContent = `Last access: ${formatRelativeTime(
  ...
  if (success) {
    if (!lab.teacher) {
      addActivity(
        "danger",
        "Acesso Negado",
        "Nenhum professor responsável definido",
        "bi-x-circle-fill"
      );
      return;
    }
  ...
  const sampleActivities = [
    {
      ...
      desc: "Painel de controle inicializado",
      icon: "bi-power",
    },
    {
      type: "info",
      title: "Laboratório Selecionado",
      desc: "Laboratório 1 selecionado",
      icon: "bi-building",
    },
    {
      type: "warning",
      title: "Professor Não Definido",
      desc: "Nenhum professor responsável definido",
  ...
  if (currentDay !== "domingo" && currentDay !== "sabado") {
    ...
    if (currentClasses) {
      currentClasses.forEach((lesson) => {
        ...
        const teacherEntry = Object.values(teachers).find(
...
function updateSchedule() {
  ...
  scheduleTable.innerHTML = classes
    .map((lesson) => {
      // Buscar o professor pelo nome no objeto teachers
...
function loadDataFromStorage() {
  try {
    ...
    teachers = savedTeachers
      ...
      : JSON.parse(JSON.stringify(defaultTeachers));

    const savedTags = localStorage.getItem("registeredTags");
    registeredTags = savedTags
      ? JSON.parse(savedTags)
      : JSON.parse(JSON.stringify(defaultRegisteredTags));
    ...
    if (savedLaboratories) {
      ...
      Object.values(laboratories).forEach((lab) => {
        if (lab.teacher && lab.teacher.rfid && teachers[lab.teacher.rfid]) {
          ...
          // Se o professor no laboratório não existe mais em teachers
          lab.teacher = null;
          lab.assignedClass = null;
        }
      });
    ...
    todayAccessCount = savedTodayAccessCount
      ? parseInt(savedTodayAccessCount)
      : 0;
    ...
    if (!window.location.pathname.includes("activity.html")) {
      // Evitar chamar em activity.html se não necessário
      loadRegisteredTags();
      populateTeacherSelect();
...
function populateTeacherSelect() {
  ...
  console.log(
    "Select de professores atualizado:",
...
function editTeacher(rfid) {
          ...
          <div class="modal-body">
            <form id="editTeacherForm">
              <div class="mb-3">
                ...
                <input type="text" class="form-control" id="editTeacherName" value="${
                  teacher.name
                }" required>
              </div>
              <div class="mb-3">
                <label class="form-label">Coordenador</label>
                <input type="text" class="form-control" id="editTeacherCoordinator" value="${
                  teacher.coordinator
                }" required>
              </div>
              <div class="mb-3">
                <label class="form-label">RFID</label>
                <input type="text" class="form-control" id="editTeacherRFID" value="${
                  teacher.rfid
                }" required>
              </div>
              <div class="mb-3">
  ...
  document
    .getElementById("editSubjectsContainer")
...
function confirmEditTeacher(oldRfid) {
  ...
  const coordinator = document
    .getElementById("editTeacherCoordinator")
    .value.trim();
  const newRfid = document
    .getElementById("editTeacherRFID")
    .value.trim()
    .toUpperCase();
  const subjectInputs = document.querySelectorAll(
    "#editSubjectsContainer .subject-input"
  );
  ...
  Object.keys(laboratories).forEach((labId) => {
    ...
    if (lab.teacher && lab.teacher.rfid === oldRfid) {
      ...
      if (
        lab.assignedClass &&
        lab.assignedClass.teacherName === oldTeacherData.name
      ) {
  ...
  Object.keys(scheduleData).forEach((day) => {
    Object.keys(scheduleData[day]).forEach((shift) => {
      scheduleData[day][shift].forEach((lesson) => {
        if (lesson.teacher === oldTeacherData.name) {
          // Comparar pelo nome antigo
...
function deleteTeacher(rfid) {
  ...
  if (!teacher) {
    console.warn(
      `Tentativa de excluir professor com RFID ${rfid} não encontrado.`
    );
    return;
  }
  ...
  if (
    !confirm(
      `Tem certeza que deseja excluir o professor ${teacher.name} (${rfid})? Esta ação removerá todas as suas informações e aulas agendadas.`
    )
  ) {
    return;
  }
...
async function initializeFirebaseData() {
  try {
    ...
    // Referência para o nó de laboratórios
    const labsRef = database.ref("laboratories");

    // Verificar se já existem dados
    const snapshot = await labsRef.get();
    if (!snapshot.exists()) {
      console.log("Nenhum dado encontrado, inicializando...");
      ...
      await labsRef.set(initialData);
      console.log("Dados iniciais criados com sucesso");
    } else {
      console.log("Dados existentes encontrados:", snapshot.val());
    }
  } catch (error) {
...
// Adicionar chamada para initializeFirebaseData no DOMContentLoaded
...
// Assegurar que as funções globais estejam disponíveis após todas as declarações
